      ...
      for(CSCCorrelatedLCTDigiCollection::DigiRangeIterator csc=corrlcts.product()->begin(); csc!=corrlcts.product()->end(); csc++)
        {
	      ...
	      int bend      = lct -> getBend(); 
              ...
              float etaG = gblEta.global_eta/126.*1.5 + 0.9;
              float phi_temp = (gblPhi.global_phi + (sector)*4096 + station*4096*12) * 1./(4*4096*12);
	      
	      float phiG = fmod(phi_temp + (sector*M_PI/3) +
                                  (M_PI*14/180.), 2*M_PI);
		...
		cout << "\n Bend      = " << bend;
		cout << "\n KeyWire   = " << keyWire;
		cout << "\n Phi Bit   = " << gblPhi.global_phi;
		cout << "\n Eta Bit   = " << gblEta.global_eta;
		cout << "\n Phi Gbl   = " << phiG;
		cout << "\n Eta Gbl   = " << etaG;
